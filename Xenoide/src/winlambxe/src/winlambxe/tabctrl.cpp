...
{
	if (!hwnd || extraPx <= 0) return label;
	HDC hdc = GetDC(hwnd);
	if (!hdc) return label;
...
void wlx::tabctrl::_refresh_padding()
{
	...
	const int extraPx = this->_closable
		? (CLOSE_BTN_SIZE + CLOSE_BTN_MARGIN * 2)
		: TAB_RIGHT_PAD;
...
void wlx::tabctrl::_draw_close_x(HDC hdc, const RECT& r, COLORREF stroke) noexcept
{
	...
	MoveToEx(hdc, x2, y1, nullptr); LineTo(hdc, x1, y2);
...
wlx::tabctrl& wlx::tabctrl::add_tab(const TCHAR* label)
{
	...
	TCITEM tci{};
	tci.mask    = TCIF_TEXT;
	tci.pszText = const_cast<TCHAR*>(padded.empty()
		? label
		: padded.data());
...
{
	if (this->_closable == yes) return *this;
	this->_closable = yes;
	if (!yes) {
		this->_hoveredCloseIdx = -1;
	}
...
void wlx::tabctrl::draw_item(const DRAWITEMSTRUCT& dis) noexcept
{
	...
	HFONT hFont = reinterpret_cast<HFONT>(
		SendMessage(this->_hWnd, WM_GETFONT, 0, 0));
	HFONT hOldFont = hFont
		? reinterpret_cast<HFONT>(SelectObject(hdc, hFont))
		: nullptr;

	// Label text — reserve right margin for the close button when present.
	RECT textRc = rc;
	textRc.left += 6;
	textRc.right = this->_closable
		? (rc.right - CLOSE_BTN_SIZE - CLOSE_BTN_MARGIN * 2)
		: (rc.right - 6);
	...
	const wl::tstring& label = (dis.itemID < this->_labels.size())
		? this->_labels[dis.itemID]
		: kEmptyLabel;
	DrawText(hdc, label.c_str(), -1, &textRc,
	...
	if (this->_closable) {
		...
		const bool hovered =
			(static_cast<int>(dis.itemID) == this->_hoveredCloseIdx);
