...
$project = if ($env:usage_project) { $env:usage_project } else { "all" }
...
$outputDir = if ($env:usage_output_dir) { $env:usage_output_dir } else { "" }
...
if ($folders.Count -eq 0) { Fail "--project must resolve to at least one subproject" }
...
foreach ($folder in $folders) {
    ...
    if (-not $outputDir) {
        $reportDir = "$folder/mutation/mull"
    } elseif ($folders.Count -gt 1) {
        $reportDir = Join-Path $outputDir (Split-Path -Leaf $folder)
    } else {
        $reportDir = $outputDir
    }
    ...
    if ($clean) {
        Write-Host ""
        Write-Host "=== [$folder] Cleaning build and mutation artifacts ==="
        if (Test-Path "$sourceDir/build-mutation-mull") { Remove-Item -Recurse -Force "$sourceDir/build-mutation-mull" }
    ...
    $buildDir = Join-Path $sourceDir "build-mutation-mull/$config"
    ...
    Write-Host ""
    Write-Host "=== [$folder / $config] Installing Conan dependencies ==="
    ...
    conan install $sourceDir --build=missing -s build_type=$config -pr:h $profile -pr:b $profile -of $buildDir `
    ...
    Write-Host "=== [$folder / $config] Configuring CMake with Mull instrumentation ==="
    ...
    Write-Host "=== [$folder / $config] Building test targets ==="
    ...
    $dbPath = Join-Path $reportDir "$reportName.sqlite"
    ...
    Write-Host ""
    Write-Host "=== [$folder] Running mull-runner on $($runExecutables.Count) test executable(s) ==="
    ...
    $reporterArgs = @("--report-dir", $reportDir, "--report-name", $reportName)
    if ($threshold) {
    ...
    Write-Host ""
    Write-Host "=== [$folder] Generating consolidated reports ==="
    & $mullReporter @reporterArgs @reporterCliArgs $dbPath
    if ($LASTEXITCODE -ne 0) { exit 1 }

    Write-Host ""
    Write-Host "=== [$folder] Mutation testing artifacts ==="
    Write-Host "  database : $dbPath"
    Write-Host "  reports  : $reportDir (report name: $reportName)"
}
exit 0
