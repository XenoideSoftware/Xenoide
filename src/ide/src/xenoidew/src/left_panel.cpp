...
LeftPanel::LeftPanel() {
    ...
    this->on_message(WM_CREATE, [this](wl::params) -> LRESULT {
        ...
        this->_treeview.create(this->hwnd(), IDC_TREEVIEW_INNER, {0, 0}, {rc.right, rc.bottom});

        if (this->_pendingImageList) {
            TreeView_SetImageList(this->_treeview.hwnd(), this->_pendingImageList, TVSIL_NORMAL);
        }
    ...
    this->on_message(WM_SIZE, [this](wl::params p) -> LRESULT {
        ...
        if (this->_treeview.hwnd()) {
            SetWindowPos(this->_treeview.hwnd(), nullptr, 0, 0, cx, cy, SWP_NOZORDER | SWP_NOACTIVATE);
        }
        return 0;
    });
}
