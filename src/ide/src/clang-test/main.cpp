...
{
    switch (k) {
    case CXCursor_Namespace:            return L"[namespace]";
    case CXCursor_ClassDecl:            return L"[class]";
    case CXCursor_StructDecl:           return L"[struct]";
    case CXCursor_UnionDecl:            return L"[union]";
    case CXCursor_EnumDecl:             return L"[enum]";
    case CXCursor_EnumConstantDecl:     return L"[const]";
    case CXCursor_FunctionDecl:         return L"[function]";
    case CXCursor_CXXMethod:            return L"[method]";
    case CXCursor_Constructor:          return L"[ctor]";
    case CXCursor_Destructor:           return L"[dtor]";
    case CXCursor_ConversionFunction:   return L"[conv]";
    case CXCursor_FieldDecl:            return L"[field]";
    case CXCursor_VarDecl:              return L"[var]";
    case CXCursor_TypedefDecl:          return L"[typedef]";
    case CXCursor_TypeAliasDecl:        return L"[using]";
    case CXCursor_FunctionTemplate:     return L"[fn tmpl]";
    case CXCursor_ClassTemplate:        return L"[class tmpl]";
    case CXCursor_ClassTemplatePartialSpecialization: return L"[partial spec]";
...
static CXChildVisitResult Visit(CXCursor cursor, CXCursor /*parent*/, CXClientData raw)
{
    ...
    if (spelling.empty()) spelling = L"<anonymous>";
...
static void ParseFile(const wchar_t* wpath)
{
    ...
    if (g_tu) { clang_disposeTranslationUnit(g_tu); g_tu = nullptr; }

    TreeView_DeleteAllItems(g_hTree);

    const char* clangArgs[] = { "-std=c++20", "-x", "c++" };
    g_tu = clang_parseTranslationUnit(
        g_index, path.c_str(),
        clangArgs, 3,
        nullptr, 0,
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nCmdShow)
{
    ...
    g_hWnd = CreateWindowExW(
        0, L"ClangAstViewer", L"Clang AST Viewer",
        WS_OVERLAPPEDWINDOW,
    ...
    if (g_index) clang_disposeIndex(g_index);
