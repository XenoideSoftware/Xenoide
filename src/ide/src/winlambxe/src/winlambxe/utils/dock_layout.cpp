...
namespace wlx {
    ...
    dock_layout::dock_layout() {
        ...
        this->on_message(WM_SETCURSOR, [this](wl::params p) -> LRESULT {
            ...
            return DefWindowProc(this->hwnd(), p.message, p.wParam, p.lParam);
        });
        ...
        this->on_message(WM_MOUSEMOVE, [this](wl::params p) -> LRESULT {
            ...
            if (_drag == HitZone::VBar) {
                _leftWidth = std::clamp(x, MIN_LEFT, _client_w - MIN_CENTER - BAR_THICK);
            } else {
                const int newBottomH = _client_h - y - BAR_THICK;
                _bottomHeight = std::clamp(newBottomH, MIN_BOTTOM, _client_h - MIN_CENTER - BAR_THICK);
            }
            this->_layout();
            return 0;
        });
    ...
    void dock_layout::show_left(bool visible) {
        ...
        if (_left)
            ShowWindow(_left, visible ? SW_SHOW : SW_HIDE);
        this->_layout();
    }
    ...
    void dock_layout::show_bottom(bool visible) {
        ...
        if (_bottom)
            ShowWindow(_bottom, visible ? SW_SHOW : SW_HIDE);
        this->_layout();
    }
    ...
    */
    ...
    dock_layout::HitZone dock_layout::_hit_test(int x, int y) const noexcept {
        ...
        if (_leftVisible && x >= leftW && x < leftW + BAR_THICK && y >= 0 && y < _client_h) {
            return HitZone::VBar;
        }

        const int hbarY = _client_h - bottomH - BAR_THICK;
        if (_bottomVisible && x >= rightX && x < _client_w && y >= hbarY && y < hbarY + BAR_THICK) {
            return HitZone::HBar;
        }
    ...
    void dock_layout::_layout() noexcept {
        if (!this->hwnd() || _client_w <= 0 || _client_h <= 0)
            return;
        ...
        if (_left && _leftVisible) {
            dwp.defer(_left, nullptr, 0, 0, leftW, _client_h, SWP_NOZORDER | SWP_NOACTIVATE);
        }
        if (_center) {
            dwp.defer(_center, nullptr, rightX, 0, rightW, centerH, SWP_NOZORDER | SWP_NOACTIVATE);
        }
        if (_bottom && _bottomVisible) {
            dwp.defer(_bottom, nullptr, rightX, bottomY, rightW, bottomH, SWP_NOZORDER | SWP_NOACTIVATE);
        }
    }
