...
| `mise run export-recipes` | Export local recipes in `conan/recipes/*` to cache | None |
...
#### `export-recipes`
- **Description**: Scans `conan/recipes/` for local Conan recipes and exports them into the local Conan cache.
