...
template <typename StrType> StrType join(std::vector<StrType> const &patterns, StrType const &sep) {
    return std::accumulate(patterns.begin(), patterns.end(), StrType{}, [sep](const auto value1, auto value2) {
        if (value1.empty()) {
            return value2;
        }
